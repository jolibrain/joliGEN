...
class SCModel(BaseDiffusionModel):
    @staticmethod
    def modify_commandline_options(parser, is_train=True):
        ...
        if is_train:
            parser = SCModel.modify_commandline_options_train(parser)
    ...
    def __init__(self, opt, rank):
        ...
        self.total_t = (
            self.opt.alg_sc_num_steps * self.opt.train_batch_size
        )  # scaled with bs
        ...
        elif self.opt.model_type == "sc":
            base_names = ["gt_image_", "y_t_", "mask_"]
            step_outputs = [
                f"output_{ts}_steps_" for ts in self.opt.alg_sc_denoise_timesteps
            ]
        ...
        print("Setting SC current_iter to", self.netG_A.current_t)
        ...
        # perceptual losses
        if "LPIPS" in self.opt.alg_sc_perceptual_loss:
            self.criterionLPIPS = LPIPS().to(self.device)
        if "DISTS" in self.opt.alg_sc_perceptual_loss:
            self.criterionDISTS = DISTS(
                mean=self.opt.alg_sc_dists_mean, std=self.opt.alg_sc_dists_std
            ).to(self.device)
    ...
    def compute_sc_loss(self):
        ...
        v_pred, v_target, t, dt_base, x_t, info = self.netG_A(y_0, mask, y_cond)
        ...
        if self.loss_G_perceptual_lpips > 0 or self.loss_G_perceptual_dists > 0:
            self.loss_G_perceptual = self.opt.alg_sc_lambda_perceptual * (
    ...
    def inference(self, nb_imgs, offset=0):
        ...
        if self.task == "pix2pix":  #
            ...
            out_shape[1] = netG.sc_model.out_channel
            ...
            if (
                ...
                for i in range(self.nb_classes_inference):
                    ...
                    self.output = netG.restoration(
                        y_t, y_cond, self.opt.alg_sc_denoise_timesteps, cur_mask
                    )
