...
class SCModel(BaseDiffusionModel):
    ...
    def __init__(self, opt, rank):
        ...
        self.gen_visual_names = [
            "gt_image_",  # Ground-truth clean image
            "y_t_",  # Input (possibly noisy image)
            "output_",  # Model output (restored/generated)
            "mask_",  #  mask
        ]
        if (
            self.opt.alg_diffusion_cond_embed != ""
    ...
    def inference(self, nb_imgs, offset=0):
        ...
        if self.task in ["inpainting"]:
            if (
                ...
                if self.opt.G_netG == "unet_vid" and self.opt.alg_sc_metric_mask:
