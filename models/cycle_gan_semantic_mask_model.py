...
class CycleGANSemanticMaskModel(CycleGANModel):
    ...
    def compute_G_loss(self):
        ...
        # semantic loss AB
        self.loss_sem_AB = self.opt.train_sem_lambda*self.criterionf_s(self.pfB, self.input_A_label)

        # semantic loss BA
        if hasattr(self, 'input_B_label'):
            self.loss_sem_BA = self.opt.train_sem_lambda*self.criterionf_s(self.pfA, self.input_B_label)#.squeeze(1))
        else:
            self.loss_sem_BA = self.opt.train_sem_lambda*self.criterionf_s(self.pfA, self.gt_pred_B)#.squeeze(1))
