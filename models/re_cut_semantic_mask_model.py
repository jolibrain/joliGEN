...
class ReCUTSemanticMaskModel(CUTSemanticMaskModel):
    ...
    def __init__(self, opt):
        ...
        self.optimizer_P = torch.optim.Adam(itertools.chain(self.netP_B.parameters()),lr=opt.alg_re_P_lr, betas=(opt.train_beta1, opt.train_beta2))
    ...
    def compute_G_loss(self):
        ...
        if not hasattr(self, 'loss_proj_real_B') or (self.loss_proj_real_B) > self.opt.alg_re_projection_threshold: #if P networks aren't accurate enough on real images, we don't use them on fake images:
            self.loss_recut = 0
            self.loss_proj_fake_B_adversarial = 0
