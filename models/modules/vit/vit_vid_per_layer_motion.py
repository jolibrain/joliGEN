...
class JiTViD(nn.Module):
    ...
    def _apply_motion_on_patches(self, x, B, F, Hp, Wp, mm, L_prefix):
        """
        x: (B*F, L_prefix + Hp*Wp, D) if prefix exists else (B*F, Hp*Wp, D)
        Apply temporal motion ONLY to patch tokens, then re-attach prefix tokens.
        """
        if L_prefix > 0:
            prefix = x[:, :L_prefix]  # (BF, L, D)
            patches = x[:, L_prefix:]  # (BF, Hp*Wp, D)
        else:
            prefix = None
            patches = x  # (BF, Hp*Wp, D)
    ...
    def initialize_weights(self):
        ...
        if hasattr(self, "motion_modules"):
            for mm in self.motion_modules:
                mm.temporal_transformer.proj_out = zero_module(
                    mm.temporal_transformer.proj_out
                )
    ...
    def forward(self, x, t, y):
        ...
        inserted = False  # whether prefix tokens are currently present

        for i, block in enumerate(self.blocks):
            # Insert in-context tokens once at in_context_start
            ...
            # ---- Motion insertion policy ----
            # apply motion at layers: motion_every-1, 2*motion_every-1, ...
            if ((i + 1) % self.motion_every) == 0:
                mm_idx = i // self.motion_every  # maps layer -> motion module index
                L_prefix = (
                    self.in_context_len if (inserted and self.in_context_len > 0) else 0
                )
                x = self._apply_motion_on_patches(
                    x, B, F, Hp, Wp, self.motion_modules[mm_idx], L_prefix
                )
        # remove prefix tokens before final prediction
        if self.in_context_len > 0:
