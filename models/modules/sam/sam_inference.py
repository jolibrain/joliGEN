...
def predict_sam(img, sam_predictor, bbox=None, use_fast_sam=False):
    ...
    if not use_fast_sam:
        ...
        best_mask = fastSAM(
            sam_predictor,
            img,
            box_prompt=prompt_bbox,
            point_prompt=point_coord,
            point_label=point_label,
        )[0]
        print(best_mask.shape)
        print(best_mask)
        best_mask = torch.tensor(best_mask)
...
def predict_sam_edges(
    ...
    for k in range(len(image)):
        ...
        for mask in batched_output[k]["non_redundant_masks"]:
            print(mask.shape, image[k].shape)
            assert (
