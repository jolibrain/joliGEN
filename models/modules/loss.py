...
class DualDiscriminatorGANLoss(DiscriminatorLoss):
    """
    Unet loss which includes loss from encoder and decoder, reference is 2002.12655
    """
    ...
    def compute_loss_D(self, netD, real, fake, fake_2):
        ...
        loss_D_pixel = loss_pred_real_pixel + loss_D_fake_pixel
        loss_D_bottleneck = loss_pred_real_bottleneck + loss_D_fake_bottleneck

        print("loss.py D loss of the pixel(decoder) real {} and fake {}, so the total is {} ".format( loss_pred_real_pixel, loss_D_fake_pixel,loss_D_pixel))
        print(
            "loss.py D loss of the bottleneck(encoder) real {}  and fake {}, so the total is {}".format( loss_pred_real_bottleneck, loss_D_fake_bottleneck,  loss_D_bottleneck)
        )
    ...
    def compute_loss_G(self, netD, real, fake):
        ...
        print("loss.py G fake loss of the pixel(decoder)  is {}".format(loss_G_pixel))
        print(
            "loss.py G fake loss of the bottleneck(encoder)  is {}".format(loss_G_bottleneck)
        )
        return loss_D_fake
