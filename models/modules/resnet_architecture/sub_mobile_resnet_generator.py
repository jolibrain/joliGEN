...
class MobileResnetBlock(nn.Module):
    def __init__(self, ic, oc, padding_type, norm_layer, dropout_rate, use_bias):
        ...
        self.conv_block = self.build_conv_block(
            ic, oc, padding_type, norm_layer, dropout_rate, use_bias
        )

    def build_conv_block(
        self, ic, oc, padding_type, norm_layer, dropout_rate, use_bias
    ):
        conv_block = []
        p = 0
        ...
        conv_block += [
            SeparableConv2d(
                in_channels=ic, out_channels=oc, kernel_size=3, padding=p, stride=1
            ),
            norm_layer(oc),
            nn.ReLU(True),
        ]
        ...
        conv_block += [
            SeparableConv2d(
                in_channels=oc, out_channels=ic, kernel_size=3, padding=p, stride=1
            ),
            norm_layer(oc),
        ]
...
class SubMobileResnetGenerator(BaseNetwork):
    def __init__(
        ...
        model = [
            ...
            nn.Conv2d(
                input_nc, config["channels"][0], kernel_size=7, padding=0, bias=use_bias
            ),
