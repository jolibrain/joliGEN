...
from .hat_util import to_2tuple, trunc_normal_
...
class HAT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.upsampler == "pixelshuffle":
            # for classical SR
            x = self.conv_first(x)
            xf, _ = self.forward_features(x)
            x = self.conv_after_body(xf) + x
    ...
    def get_feats(self, x, extract_layer_ids=[]):
        ...
        nfeats = []
        for f in feats:
            # reshape from [N, B, C] to [N, H, W, C] with H=img_size
            f = torch.reshape(f, (f.size(0), self.img_size, self.img_size, -1))
            nfeats.append(f)
        feats = nfeats
        nfeats = feats
        return feats
