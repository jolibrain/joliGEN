...
from models.modules.mmcv_clean.config import load_config_file
...
class SegformerGeneratorDiff_attn(BaseGenerator_attn):
    ...
    def __init__(
        ...
        # cfg = Config.fromfile(os.path.join(jg_dir, G_config_segformer))
        cfg = load_config_file(os.path.join(jg_dir, G_config_segformer))
        cfg["backbone"]["in_channels"] = 2 * input_nc + 1  # for embedding
