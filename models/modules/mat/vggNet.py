...
class VGGFeatureExtractor(nn.Module):
    ...
    def __init__(
        ...
        model_ctor = getattr(vgg, vgg_type)
        weights_enum = getattr(vgg, f"{vgg_type.upper()}_Weights", None)
        if weights_enum is not None:
            features = model_ctor(weights=weights_enum.DEFAULT).features[: max_idx + 1]
        else:
            features = model_ctor(pretrained=True).features[: max_idx + 1]
