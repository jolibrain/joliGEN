...
def load_model(
    ...
    if opt.model_type in ["cm", "cm_gan", "b2b", "b2b_gan"]:
    ...
    if opt.model_type in ["b2b", "b2b_gan"] and weights_path.endswith(".pth"):
...
def generate(
    ...
    **unused_options,
    ...
    # noise aug
    B, C, H, W = y0_tensor_list[0].shape
    k_ctx = seq_half  # number of noisy context frames

    # noise parameters
    num_buckets = 2
    max_sigma = 0.71

    # Sample ONE noise level per sequence (same for all context frames)
    bucket_idx = torch.randint(0, num_buckets, (B,), device=y0_tensor_list[0].device)
    sigma_values = bucket_idx.float() / (num_buckets - 1) * max_sigma
    sigma = sigma_values.view(B, 1, 1, 1)  # [B,1,1,1]
    #    sigma = torch.rand((B, 1, 1, 1), device=y0_tensor_list[0].device) * max_sigma  #debug,del later

    eps_ctx = torch.randn(B, C, H, W, device=y0_tensor_list[0].device)

    y0_noisy_list = []
    for t, frame in enumerate(y0_tensor_list):
        frame_noisy = frame.clone()
        if t < k_ctx:
            # Apply same noise pattern and level to context frames
            frame_noisy = frame + sigma * eps_ctx
        y0_noisy_list.append(frame_noisy)

    # ===============================
    # step 2: autoregressive inference
    # ===============================
    inputmix = True
    input3Frame = [f.clone().detach() for f in y0_noisy_list[:seq_half]]
    ...
    # Forward temporal sliding window
    # --------------------------------
    # seq_half = 3 → each window = 6 frames (context + predict)
    # slide_window = len(N) // 3 - 1  → overlapping 3-frame steps
    for slide in range(slide_window):
        window_start = slide * seq_half
        window_end = window_start + seq_half * 2

        # Prepare input tensors for this window
        if slide == 0:
            # First window: use original frames only
            y_t = prepare_tensors(y_t_list[window_start:window_end])
            cond_image = prepare_tensors(cond_image_list[window_start:window_end])
            y0_tensor = prepare_tensors(y0_tensor_list[window_start:window_end])
            mask = prepare_tensors(mask_list[window_start:window_end])
            if inputmix:
                cond_image = prepare_tensors(
                    y0_noisy_list[:seq_half]
                    + cond_image_list[window_start + seq_half : window_end]
                )
                cond_image_list[:seq_half] = [f.clone().detach() for f in input3Frame]
            else:
                cond_image = prepare_tensors(
                    y0_tensor_list[:seq_half]
                    + cond_image_list[window_start + seq_half : window_end]
                )

        else:
            # Subsequent windows: reuse last generated frames as new conditions
            new_start = (slide + 1) * seq_half
            new_end = new_start + seq_half

            input_y_t = last_seq_half_y_t + y_t_list[new_start:new_end]
            input_cond_image = (
                last_seq_half_cond_image + cond_image_list[new_start:new_end]
            )
            input_y0_tensor = (
                last_seq_half_y0_tensor + y0_tensor_list[new_start:new_end]
            )
            input_mask = last_seq_half_mask + mask_list[new_start:new_end]
        ...
        with torch.no_grad():
            ...
            elif opt.model_type in ["b2b", "b2b_gan"]:
                B = y_t.size(0)
        ...
        if slide == 0:
            final_outputs = out_img_temp_list_batch.copy()
            if inputmix:
                new_cond_list = (
                    y0_noisy_list[:seq_half] + out_img_tensor_list_batch[-seq_half:]
                )
            else:
                new_cond_list = (
                    cond_image_list[:seq_half] + out_img_tensor_list_batch[-seq_half:]
                )

        else:
            final_outputs.extend(out_img_temp_list_batch[-seq_half:])
            new_cond_list.extend(out_img_tensor_list_batch[-seq_half:])

        # Update autoregressive state
        y_t_temp_list = separate_tensors(y_t)
        y0_tensor_temp_list = separate_tensors(y0_tensor)
    ...
    # Final output
    out_img_list = final_outputs

    if logger:
        logger.info(
            f"[it: %i/%i] - [3/%i] autoregressive generation completed"
            % (iteration, nb_samples, PROGRESS_NUM_STEPS)
        )
