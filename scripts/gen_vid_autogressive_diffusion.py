...
def generate(
    ...
    **unused_options,
    ...
    image_bbox_pairs.sort(key=lambda x: natural_keys(x[0]), reverse=True)
    paths_length = len(image_bbox_pairs)

    startframe = random.randint(0, paths_length % 4)
    limited_image_bbox_pairs = image_bbox_pairs[
        startframe : startframe + (paths_length // 4) * 4
    ]
    ...
    for img_path, bbox_path in zip(limited_paths_img, limited_paths_bbox):
        ...
        sequence_count += 1
    print("finish step1 tensor shape ", y_t_list[-1].shape)
    print(" totally path number ", paths_length)
    print("total slide  window ", slide_window)
    print("y_t_list", len(y_t_list))
    # finish step 1 with limited_paths_img
    # step2 Loop through the sliding window
    new_cond_list = []

    for slide in range(slide_window):  # slide_window is paths_length // 4 - 1
        print("******befroe model slide ", slide)
        if slide == 0:
            ...
            print("after prepare_tensor ", y_t.shape)
            print("********before model tensor shape ", y_t.shape)
        else:
            # Calculate the starting frame for subsequent slides
            start_frame = slide * 4
            # Prepare inputs for the model
            input_y_t = last_4_frame_y_t + y_t_list[start_frame + 4 : start_frame + 8]
            input_cond_image = (
                last_4_frame_cond_image
                + cond_image_list[start_frame + 4 : start_frame + 8]
            )
            input_y0_tensor = (
                last_4_frame_y0_tensor
                + y0_tensor_list[start_frame + 4 : start_frame + 8]
            )
            input_mask = (
                last_4_frame_mask + mask_list[start_frame + 4 : start_frame + 8]
            )

            # Stack tensors for model input
            print("*******before prepare_tensors shape ", input_y_t[0].shape)
        ...
        print("****** before model y_t", type(y_t), y_t.shape)
        ...
        if slide == 0:
            last_4_frame_y_t = y_t_list[4:8]
            last_4_frame_cond_image = out_img_tensor_list[4:8]
            last_4_frame_y0_tensor = y0_tensor_list[4:8]
            last_4_frame_mask = mask_list[4:8]
            out_img_list = out_img_temp_list[:4]
            new_cond_list = cond_image_list[:4]
            print("!!!after model out_img_list len", len(out_img_list))
            print(
                "!!!! after model  last_4_frame_mask [-1]",
                slide,
                last_4_frame_y_t[0].shape,
            )

        else:
            print("!!!!after model slide >0 y_t shape ", slide, y_t.shape)
            y_t_temp_list = separate_tensors(y_t)
            last_4_frame_y_t = y_t_temp_list[-4:]
            print(
                "!!!separate_tensor , list len, shape ",
                len(last_4_frame_y_t),
                last_4_frame_y_t[0].shape,
            )
            last_4_frame_cond_image = out_img_tensor_list[-4:]

            y0_tensor_temp_list = separate_tensors(y0_tensor)
            last_4_frame_y0_tensor = y0_tensor_temp_list[-4:]

            mask_temp_list = separate_tensors(mask)
            last_4_frame_mask = mask_temp_list[-4:]

            print("out_img_temp_list ", len(out_img_temp_list))
            out_img_list.extend(out_img_temp_list[:4])
            new_cond_list.extend(out_img_tensor_list[-4:])
            print("new_cond_list ", len(new_cond_list))
            print("!!!after model out_img_list len if ", len(out_img_list))

    out_img_list.extend(out_img_temp_list[-4:])
    new_cond_list.extend(cond_image_list[-4:])
