...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-in", help="data input txt file with image and bbox files", required=True)
    parser.add_argument("--bbox-use", default=-1, type=int, help="bbox id to sample from (default all)")
    parser.add_argument(
    ...
    parser.add_argument('--nimgs', default=1, type=int, help='number of different images to iterate')
    parser.add_argument('--nsamples', default=1, type=int, help='number of samples per image')

    parser.add_argument('--transfer', action='store_true', help='wether to transfer conditioning from another image')
    parser.add_argument('--source-img', default='PanneauToulouse.png', help="image to use as conditionment")
    parser.add_argument('--source-bbox', default=None, help="bbox file for image tu use as conditionment")
    parser.add_argument('--source-id', type=int, default=0, help="line of bbox file tu use")

    parser.add_argument("--create-collage", action='store_true', help='wether to create a collage to compare generations')
    ...
    for img_nb, line in enumerate(lines):
        ...
        for i in range(args.nimgs):
            ...
            for j in range(0, bbox_count):
                ...
                args.img_in = elts[0]
                args.bbox_in = elts[1]
                args.img_out = os.path.join(args.dir_out,imgname.replace('.jpg','') + '-' + str(j) + '.jpg')
                ...
                if args.create_collage:
                    ...
                    img_paths = [os.path.join(args.dir_out, args.name + ext) for ext in extensions]
