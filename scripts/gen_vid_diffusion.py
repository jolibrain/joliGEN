...
def generate(
    ...
    **unused_options,
    ...
    additional_frame = 10  # sum of opt.data_temporal_number_frames/additional_frame should less than option G_unet_vid_max_frame
    ...
    for img_path, bbox_path in zip(limited_paths_img, limited_paths_bbox):
        img_in = os.path.join(
            os.path.dirname(os.path.dirname(args.paths_file)), img_path
        )
        mask_in = os.path.join(
            os.path.dirname(os.path.dirname(args.paths_file)), bbox_path
        )
        bbox_select = None
        # reading image
...
def inference_logger(name):
    PROCESS_NAME = "gen_video_diffusion"
...
if __name__ == "__main__":
    ...
    inference_options = InferenceDiffusionOptions()
    parser = inference_options.initialize(parser)

    parser.add_argument(
        "--paths_file",
        required=True,
        help="Path to the paths file which contains pairs of image and bounding box paths",
    )
