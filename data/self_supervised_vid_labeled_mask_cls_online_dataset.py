...
class SelfSupervisedVidLabeledMaskClsOnlineDataset(BaseDataset):
    def __len__(self):
    ...
    def get_img(
        ...
        for i in range(self.num_frames):
            ...
            try:
                ...
                if i == 0:
                    ...
                    # self.cat_A_ref_bbox = ref_A_bbox[0]  ###???del  ?
        ...
        #        print("in the return dataloader set  result")
        #        if isinstance(result, dict):
        #            print("Keys in the batch:", result.keys())
        ...
        #
        ########
        # --- Debug: save images before return (with de-normalization) ---
        import cv2, numpy as np
        from pathlib import Path

        debug_dir = Path("viz_debug")
        debug_dir.mkdir(parents=True, exist_ok=True)

        # Try to detect & undo common normalizations
        IMAGENET_MEAN = np.array([0.485, 0.456, 0.406], dtype=np.float32)
        IMAGENET_STD = np.array([0.229, 0.224, 0.225], dtype=np.float32)

        def denorm_np(img_chw: np.ndarray) -> np.ndarray:
            """
            img_chw: float32 numpy [C,H,W], possibly normalized.
            ...
            # Heuristics:
            cmin, cmax = float(img.min()), float(img.max())

            # Case A: already 0..1
            if -1e-4 <= cmin and cmax <= 1.0 + 1e-4:
                # Could still be [-1,1]—detect by negative min
                if cmin < -0.01:
            ...
            # Case B: looks like ImageNet norm (mean/std)
            # If values roughly centered near 0, try inverse of ImageNet norm
            if -3.5 <= cmin <= 3.5 and -3.5 <= cmax <= 3.5:
                img = (img * IMAGENET_STD) + IMAGENET_MEAN
                img = np.clip(img, 0.0, 1.0)
                return img

            # Case C: looks like 0..255
        ...
        def to_uint8_rgb(t):
            """Tensor [C,H,W] -> uint8 [H,W,C] in RGB."""
            arr = t.detach().cpu().float().numpy()
            img01 = denorm_np(arr)  # [H,W,C] in 0..1
            img255 = (img01 * 255.0).round().astype(np.uint8)
            return img255

        def to_uint8_gray(t):
            """Tensor [H,W] or [1,H,W] -> uint8 [H,W]."""
            arr = t.detach().cpu().float()
            if arr.ndim == 3 and arr.shape[0] == 1:
                arr = arr[0]
            arr = arr.numpy()
            # If it looks like 0..1, scale; if already 0..255, clip.
            if arr.max() <= 1.0 + 1e-6:
        ...
        #        # Save A_ref (RGB)
        #        cv2.imwrite(
        #            str(debug_dir / "A_ref.png"),
        #            cv2.cvtColor(to_uint8_rgb(result["A_ref"]), cv2.COLOR_RGB2BGR),
        #        )
        #        # Save all A frames (RGB)
        #        for i, frame in enumerate(result["A"]):  # [T,C,H,W]
        #            img = to_uint8_rgb(frame)
        #            cv2.imwrite(str(debug_dir / f"A_frame{i:02d}.png"),
        #                        cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        #
        #        # Save A_ref_label_mask (GRAY)
        #        cv2.imwrite(
        #            str(debug_dir / "A_ref_label_mask.png"),
        #            to_uint8_gray(result["A_ref_label_mask"])
        #        )
        #
        #        # Save all A_label_mask frames (GRAY)
        #        for i, mask_t in enumerate(result["A_label_mask"]):  # [T,1(or C),H,W]
        #            cv2.imwrite(str(debug_dir / f"A_label_mask{i:02d}.png"),
        #                        to_uint8_gray(mask_t))
        ##########
        #                # Save B_ref (RGB)
        #        cv2.imwrite(
        #            str(debug_dir / "B_ref.png"),
        #            cv2.cvtColor(to_uint8_rgb(result["B_ref"]), cv2.COLOR_RGB2BGR),
        #        )
        #        # Save all A frames (RGB)
        #        for i, frame in enumerate(result["B"]):  # [T,C,H,W]
        #            img = to_uint8_rgb(frame)
        #            cv2.imwrite(str(debug_dir / f"B_frame{i:02d}.png"),
        #                        cv2.cvtColor(img, cv2.COLOR_RGB2BGR))
        #
        #        # Save A_ref_label_mask (GRAY)
        #        cv2.imwrite(
        #            str(debug_dir / "B_ref_label_mask.png"),
        #            to_uint8_gray(result["B_ref_label_mask"])
        #        )
        #
        #        # Save all A_label_mask frames (GRAY)
        #        for i, mask_t in enumerate(result["B_label_mask"]):  # [T,1(or C),H,W]
        #            cv2.imwrite(str(debug_dir / f"B_label_mask{i:02d}.png"),
        #                        to_uint8_gray(mask_t))
        #
        #
        #        print(f"[debug] Saved images to {debug_dir.resolve()}")
        ##########
        #
